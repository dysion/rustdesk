...
class PlatformFFI {
  ...
  JdeskImpl get ffiBind => _ffiBind;
  ...
  void _startListenEvent(JdeskImpl jdeskImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = jdeskImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
